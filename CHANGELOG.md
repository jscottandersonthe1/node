...
* **tools**: Include a tick processor in core, exposed via the `--prof-process` commandline argument which can be used to process V8 profiling output files generated when using the `--prof` commandline argument. (Matt Loring) [#4021](https://github.com/nodejs/node/pull/4021).
